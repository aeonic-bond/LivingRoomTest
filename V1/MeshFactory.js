...
const MeshFactory = {
  ...
  build(config, item) {
    ...
    } else if (config.mesh.type === 'roundedBox') {
      this._buildRoundedBox(group, config, mats);
      group.rotation.y = item.rotation || 0;
  ...
  _buildRoundedBox(group, config, mats) {
    const m = config.mesh;
    const geo = this._roundedBoxGeo(m.w, m.d, m.h, m.r);
